...
| 12 | Removing secrets from git history | [12-removing-secrets-from-history.md](12-removing-secrets-from-history.md) | Gitleaks |

## `pre-commit-hook/`

Ready-to-copy files that stop secrets **before** they're ever committed:

- `pre-commit` — plain git hook (zero dependencies); copy to `.git/hooks/`
- `.pre-commit-config.yaml` — `pre-commit` framework config; lives in the
  repo root so the whole team shares it

See [12-removing-secrets-from-history.md](12-removing-secrets-from-history.md)
for install steps and why a new commit alone never removes a leaked secret.
